...
class EGContainer extends EGObject{
	/**
	 * @var static
	 */
	...
	public function get($class, $params = [], $config = [])
	{
		...
		if (is_callable($definition, true)) {
			...
			throw new EGException("Unexpected object definition type: " . gettype($definition));
		}
		...
		if (empty($definition)) {
			...
			if (!isset($definition['class'])) {
				if (strpos($class, '\\') !== false) {
					...
					throw new EGException("A class definition requires a \"class\" member.");
				}
			}
			return $definition;
		} else {
			throw new EGException("Unsupported definition type for \"$class\": " . gettype($definition));
		}
	}
		...
		if ($constructor !== null) {
			foreach ($constructor->getParameters() as $param) {
				if ($param->isDefaultValueAvailable()) {
					...
					$c = $param->getClass();
					$dependencies[] = EGInstance::of($c === null ? null : $c->getName());
				}
			}
		}
	...
	protected function resolveDependencies($dependencies, $reflection = null)
	{
		foreach ($dependencies as $index => $dependency) {
			if ($dependency instanceof EGInstance) {
				if ($dependency->id !== null) {
					$dependencies[$index] = $this->get($dependency->id);
				} elseif ($reflection !== null) {
					$name = $reflection->getConstructor()->getParameters()[$index]->getName();
					$class = $reflection->getName();
					throw new EGException("Missing required parameter \"$name\" when instantiating \"$class\".");
				}
			}
		}
		return $dependencies;
